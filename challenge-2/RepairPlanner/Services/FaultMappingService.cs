...
public sealed class FaultMappingService : IFaultMappingService
{
    ...
    public IReadOnlyList<string> GetRequiredSkills(string faultType)
    {
        ...
        return FaultToSkills.TryGetValue(faultType.Trim(), out var skills) && skills.Count > 0
            ? skills
    ...
    public IReadOnlyList<string> GetRequiredParts(string faultType)
    {
        ...
        return FaultToParts.TryGetValue(faultType.Trim(), out var parts)
            ? parts
            : Array.Empty<string>();
    }

    public bool IsKnownFaultType(string faultType) =>
        !string.IsNullOrWhiteSpace(faultType) && FaultToSkills.ContainsKey(faultType.Trim());
}
